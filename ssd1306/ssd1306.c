...
int ssd1306_command(const ssd1306_t *dev, uint8_t cmd) {
	I2C_DEV_TAKE_MUTEX((i2c_dev_t*)&dev->i2c_dev);
	I2C_DEV_CHECK((i2c_dev_t*)&dev->i2c_dev,
			i2c_dev_write_reg(&dev->i2c_dev, 0x00, &cmd, 1));
	I2C_DEV_GIVE_MUTEX((i2c_dev_t*)&dev->i2c_dev);
	return ESP_OK;
}
...
esp_err_t ssd1306_init_desc(ssd1306_t *dev, uint8_t addr, i2c_port_t port,
	...
	if (addr != SSD1306_I2C_ADDR_LO && addr != SSD1306_I2C_ADDR_HI) {
...
int ssd1306_init(const ssd1306_t *dev) {
	...
	switch (dev->screen) {
	case SSD1306_SCREEN:
		if (!ssd1306_display_on(dev, false) && !ssd1306_set_osc_freq(dev, 0x80)
...
int ssd1306_load_frame_buffer(const ssd1306_t *dev, uint8_t buf[]) {
	...
	for (i = 0; i < len; i++) {
		...
		I2C_DEV_TAKE_MUTEX((i2c_dev_t*)&dev->i2c_dev);
		I2C_DEV_CHECK((i2c_dev_t*)&dev->i2c_dev,
				i2c_dev_write_reg(&dev->i2c_dev, 0x40, buf ? &buf[i] : tab,
						16));
		I2C_DEV_GIVE_MUTEX((i2c_dev_t*)&dev->i2c_dev);
...
int ssd1306_display_on(const ssd1306_t *dev, bool on) {
	return ssd1306_command(dev,
			on ? SSD1306_SET_DISPLAY_ON : SSD1306_SET_DISPLAY_OFF);
}
...
int ssd1306_set_segment_remapping_enabled(const ssd1306_t *dev, bool on) {
	return ssd1306_command(dev,
			on ? SSD1306_SET_SEGMENT_REMAP1 : SSD1306_SET_SEGMENT_REMAP0);
}

int ssd1306_set_scan_direction_fwd(const ssd1306_t *dev, bool fwd) {
	return ssd1306_command(dev,
			fwd ? SSD1306_SET_SCAN_DIR_FWD : SSD1306_SET_SCAN_DIR_BWD);
}
...
int ssd1306_set_inversion(const ssd1306_t *dev, bool on) {
	return ssd1306_command(dev,
			on ? SSD1306_SET_INVERSION_ON : SSD1306_SET_INVERSION_OFF);
}
...
int ssd1306_set_whole_display_lighting(const ssd1306_t *dev, bool light) {
	return ssd1306_command(dev,
			light ? SSD1306_SET_ENTIRE_DISP_ON : SSD1306_SET_ENTIRE_DISP_OFF);
}
...
int ssd1306_load_xbm(const ssd1306_t *dev, uint8_t *xbm, uint8_t *fb) {
		...
		for (column = 0; column < dev->width / 8; column++) {
			...
			for (bit = 0; bit < 8; bit++) {
				if (*(xbm + xbm_offset) & 1 << bit) {
					*(fb + dev->width * (row / 8) + column * 8 + bit) |= 1
							<< row % 8;
				}
			}
		}
	...
	return 0;		//ssd1306_load_frame_buffer(dev, fb);
}
...
int ssd1306_fill_circle(const ssd1306_t *dev, uint8_t *fb, int8_t x0, int8_t y0,
	...
	while (y >= x) {
		...
		if (color != OLED_COLOR_INVERT) {
			if ((err = ssd1306_draw_vline(dev, fb, x0 - y, y0 - x, 2 * x + 1,
					color)))
				return err;
			if ((err = ssd1306_draw_vline(dev, fb, x0 + y, y0 - x, 2 * x + 1,
					color)))
	...
	if (color == OLED_COLOR_INVERT) {
		...
		if ((err = ssd1306_draw_hline(dev, fb, x0 - r, y0, r - x1 + 1, color)))
			return err;
		while (x >= y) {
			if ((err = ssd1306_draw_hline(dev, fb, x0 + x1, y0 - y, x - x1 + 1,
					color)))
				return err;
			if ((err = ssd1306_draw_hline(dev, fb, x0 + x1, y0 + y, x - x1 + 1,
					color)))
				return err;
			if ((err = ssd1306_draw_hline(dev, fb, x0 - x, y0 - y, x - x1 + 1,
					color)))
				return err;
			if ((err = ssd1306_draw_hline(dev, fb, x0 - x, y0 + y, x - x1 + 1,
					color)))
...
int ssd1306_draw_string(const ssd1306_t *dev, uint8_t *fb,
	...
	while (*str) {
		if ((err = ssd1306_draw_char(dev, fb, font, x, y, *str, foreground,
				background)) < 0)
			return err;
		x += err;
		++str;
...
int ssd1306_start_scroll_hori_vert(const ssd1306_t *dev, bool way,
	...
	if (!ssd1306_command(dev, 0x00)
			...
			!ssd1306_command(dev, (start & 0x07))
			&& !ssd1306_command(dev, frame)
			&& !ssd1306_command(dev, (stop & 0x07)) && !ssd1306_command(dev, dy)
			&& !ssd1306_command(dev, SSD1306_SCROLL_ENABLE)) {
		return 0;
	}
	return -EIO;
